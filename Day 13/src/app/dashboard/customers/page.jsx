        ...
        <Card>
          ...
          <CardBody>
            <form onSubmit={handleAddCustomer} style={{ marginBottom: '1.5rem', background: 'rgba(15, 23, 42, 0.6)', padding: '1rem', borderRadius: '12px' }}>
              <Input placeholder="Customer Name" value={custName} onChange={(e) => setCustName(e.target.value)} error={custErrors.name} required />
              <Input placeholder="Phone (e.g. +923001234567)" value={custPhone} onChange={(e) => setCustPhone(e.target.value)} error={custErrors.phone} icon={Phone} required />
              <Button type="submit" variant="success" style={{ width: '100%' }} icon={UserPlus}>Save Customer</Button>
            </form>
              ...
              {customers.map(c => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); handleOpenEditCustModal(c); }} style={{ background: 'transparent', border: 'none', color: '#60a5fa', cursor: 'pointer' }}><Edit size={14} /></button>
                    <button onClick={(e) => { e.stopPropagation(); handleDeleteCustomer(c.id); }} style={{ background: 'transparent', border: 'none', color: '#f87171', cursor: 'pointer' }}><Trash2 size={14} /></button>
                  </div>
                </div>
              ))}
            </div>
