...
import { 
  BookOpen, Users, ArrowDownLeft, ArrowUpRight, Plus, Trash2, Edit, 
...
export default function DashboardOverviewPage() {
  const [transactions, setTransactions] = useState([
    ...
    { id: 2, type: 'out', amount: 6200, category: 'Utilities', note: 'K-Electric Bill Payment', time: '11:15 AM' },
    { id: 3, type: 'in', amount: 15500, category: 'Sales', note: 'Wholesale Electronics Order', time: '01:45 PM' }
  ]);
    ...
    <div className="animate-fade-in">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ color: '#94a3b8', fontSize: '0.95rem' }}>
            Multi-module overview for **Hamza Electronics** with full CRUD capabilities.
          </p>
        </div>
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <Link href="/dashboard/cashbook" style={{ textDecoration: 'none' }}>
            <Button variant="primary" size="sm">Cashbook CRUD Module →</Button>
          </Link>
          <Link href="/dashboard/customers" style={{ textDecoration: 'none' }}>
            <Button variant="success" size="sm">Udhar CRUD Module →</Button>
          </Link>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem', marginBottom: '2rem' }}>
        ...
        <Card borderLeftColor="#8b5cf6">
          <div style={{ fontSize: '0.85rem', color: '#94a3b8', marginBottom: '0.3rem' }}>Pending Customer Dues</div>
          <div style={{ fontSize: '1.8rem', fontWeight: 800, color: '#c084fc' }}>Rs. 43,500</div>
        </Card>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '1.5rem', flexWrap: 'wrap' }}>
        ...
        <Card>
          <CardHeader title="Recent Activity Feed" subtitle="Today's live ledger entries" icon={BookOpen} />
          <CardBody>
            <Table
              columns={[
                { header: 'Type' },
              ...
              renderRow={(t) => (
                <>
                  ...
                  <td style={{ padding: '0.75rem 1rem', textAlign: 'center' }}>
                    <button onClick={() => handleDeleteTx(t.id)} style={{ background: 'transparent', border: 'none', color: '#64748b', cursor: 'pointer' }}>
                      <Trash2 size={16} />
                    </button>
                  </td>
                </>
