...
import { 
  ...
  Search, Filter, CheckCircle2, AlertTriangle, RefreshCw
} from 'lucide-react';
...
import { LoadingState, EmptyState, ErrorState } from '../../../components/ui/StateAlert';
import { validateAmount, validateRequired } from '../../../lib/validation';

export default function CashbookCRUDPage() {
  // Cash Transactions CRUD State
  const [transactions, setTransactions] = useState([
    { id: 1, type: 'in', amount: 28000, category: 'Sales', note: 'Morning Counter Electronics Sale', time: '09:30 AM' },
    { id: 2, type: 'out', amount: 6200, category: 'Utilities', note: 'Shop Electricity Bill', time: '11:15 AM' },
    { id: 3, type: 'in', amount: 15500, category: 'Sales', note: 'Wholesale Accessories Supply', time: '01:45 PM' },
    { id: 4, type: 'out', amount: 9000, category: 'Inventory', note: 'New Mobile Covers Stock Advance', time: '04:20 PM' }
  ]);
  ...
  // Application State Simulator Toggles (Loading / Error)
  ...
  // --- CRUD ACTION 2: EDIT ENTRY (PREPARE MODAL) ---
  ...
  // --- CRUD ACTION 3: UPDATE ENTRY (SUBMIT MODAL) ---
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <Badge variant="blue" style={{ marginBottom: '0.4rem' }}>Day 11 Full CRUD Module</Badge>
          <h1 style={{ fontSize: '1.8rem', fontWeight: 800, color: 'white' }}>Digital Cashbook (Create, Read, Edit, Delete)</h1>
          <p style={{ color: '#94a3b8', fontSize: '0.95rem' }}>Full transaction lifecycle management with Edit Modals and Loading/Empty/Error UI states.</p>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          ...
          <Button variant="danger" size="sm" onClick={() => setApiErrorMessage('Simulated API Connection Timeout — Please check internet connection.')}>
      ...
      <ErrorState message={apiErrorMessage} onDismiss={() => setApiErrorMessage(null)} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem', marginBottom: '2rem' }}>
        ...
        <Card borderLeftColor="#3b82f6">
          <div style={{ fontSize: '0.85rem', color: '#94a3b8', marginBottom: '0.3rem' }}>Net Running Cash</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: '1.5rem', flexWrap: 'wrap' }}>
        ...
        {/* CREATE FORM CARD */}
        <Card>
          <CardHeader title="Create New Entry" subtitle="Form validation enabled" icon={Plus} />
          <CardBody>
            <form onSubmit={handleAddTransaction}>
              ...
              <Input
                ...
                type="text"
                placeholder="e.g. Counter sale or Shop bill"
                value={note}
                onChange={(e) => setNote(e.target.value)}
              />

              <Button type="submit" variant={entryType === 'in' ? 'success' : 'danger'} style={{ width: '100%' }} icon={Plus}>
                Add {entryType === 'in' ? 'Cash In (+)' : 'Cash Out (-)'}
              </Button>
            </form>
          </CardBody>
        </Card>

        {/* READ, EDIT, DELETE TABLE CARD */}
        <Card>
          <CardHeader
            title="Transactions Ledger"
            subtitle={`${filteredTransactions.length} entries matching search`}
            action={
              <Input
          ...
          <CardBody>
            ...
            {isLoading ? (
              <LoadingState message="Loading cashbook records..." />
            ) : (
              <Table
                columns={[
                  { header: 'Type' },
                ...
                emptyMessage="No transaction records match your search query."
                renderRow={(t) => (
                  <>
      ...
      <Modal
        ...
        title="✏️ Edit Cashbook Transaction"
      >
        <form onSubmit={handleUpdateTransaction}>
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem' }}>
            <button
              ...
              style={{
                ...
                borderRadius: '8px',
                border: 'none',
                background: editType === 'in' ? '#10b981' : 'rgba(255,255,255,0.08)',
                color: 'white',
                fontWeight: 700,
                cursor: 'pointer'
              }}
            ...
            <button
              ...
              style={{
                ...
                borderRadius: '8px',
                border: 'none',
                background: editType === 'out' ? '#ef4444' : 'rgba(255,255,255,0.08)',
                color: 'white',
                fontWeight: 700,
                cursor: 'pointer'
              }}
          ...
          <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem', justifyContent: 'flex-end' }}>
            <Button variant="secondary" onClick={() => setEditModalTx(null)}>
              Cancel
            </Button>
