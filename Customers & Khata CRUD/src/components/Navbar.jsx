...
import { BookOpen, Menu, X, Lock, LogOut, KeyRound, Database } from 'lucide-react';
...
export default function Navbar() {
  ...
  const navLinks = [
    { name: 'Home', path: '/' },
    { name: 'Customer / Khata CRUD', path: '/dashboard/customers' },
    { name: 'Digital Cashbook', path: '/dashboard/cashbook' },
    { name: 'Multi-Business', path: '/dashboard/businesses' },
      ...
      <div style={{ maxWidth: '1380px', margin: '0 auto', padding: '0.85rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', textDecoration: 'none' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '0.65rem', color: '#34d399', fontWeight: 700, letterSpacing: '0.5px', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
              <Database size={10} /> DAY 15-19: CRUD & ZOD SPECIALIST
            </div>
          </div>
        </Link>
          ...
          {isAuthenticated ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <Link href="/dashboard/customers" style={{ textDecoration: 'none' }}>
                <Button variant="primary" size="sm">Open Customers & Khata →</Button>
              </Link>
              <button
                onClick={logout}
      ...
      {mobileMenuOpen && (
        <div className="animate-fade-in" style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid rgba(255, 255, 255, 0.1)' }}>
            <Link href="/dashboard/customers" onClick={() => setMobileMenuOpen(false)} style={{ textDecoration: 'none' }}>
              <Button variant="primary" style={{ width: '100%' }}>Launch Khata Dashboard</Button>
            </Link>
          </div>
        </div>
      )}
